...
and never re-fetched once a *valid* cache exists (PRD §5). Reading and
writing the cache file itself (corrupt-cache tolerance, atomic writes) is
shared with releases.py's cache via `json_cache.py`.
"""
